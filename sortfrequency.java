...
public class sortfrequency
{
	public static void sorting(int[] inputArray) {
        ...
        for (int i = 0; i < inputArray.length; i++) 
        {
			...
			if (elementCountMap.containsKey(inputArray[i]))
            {
                ...
                elementCountMap.put((Integer) inputArray[i], elementCountMap.get(inputArray[i])+1);
            }
            else
            {
                //If element is not present, insert this element with 1 as its value
                 
                elementCountMap.put((Integer) inputArray[i], 1);
            }
        }
    ...
    {
    	sorting(new int[] {7, 1, 3, 4, 7, 1, 7, 1, 4, 5, 1, 9, 3});
    }
}
